...
"""
Step 5: Forest-style contrasts across baseline A1C categories (A1C outcome)

Audience-friendly summary:
- We fit one combined GEE model that includes time-by-A1C interactions.
- We estimate average absolute A1C change at 3, 6, 9, 12, 15, 18, 21, and 24 months
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="Step 5: Forest-style contrasts across A1C categories (A1C outcome)"
    )
    p.add_argument(
        "--input-csv",
    ...
    p.add_argument(
        "--outdir",
        default=os.path.join("output", "step5_forest_contrasts_a1c"),
        help="Directory to write outputs",
    )
    # Months: 3, 6, 9, 12, 15, 18, 21, 24 -> days: 90, 180, 270, 365, 450, 548, 630, 730
    p.add_argument(
        "--time-days",
        default="90,180,365,548",
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Adherence gap in days for gap-specific output folder (e.g., 90, 120)",
    )
...
def _fit_combined(df: pd.DataFrame, df_spline: int):
    ...
    formula = (
        f"abs_a1c_change ~ {bs_term} * baseline_a1c_category + "
        f"age_group + gender + baseline_bmi_final_category + race"
    )
    # Remove metformin_with_glp1_baseline per requirements
...
def main(argv=None):
    ...
    df = enforce_a1c_order(df, order=a1c_order, context="step5_forest_contrasts_a1c")
    ...
    # Save combined model summary, coefficients, and covariance
    with open(os.path.join(outdir, "gee_summary_combined.txt"), "w") as f:
        f.write(str(res.summary()))
    # Labeled coefficients with term names as first column
    coef_df = pd.DataFrame({
        ...
        "CI Lower": np.asarray(res.conf_int()[0], dtype=float),
        "CI Upper": np.asarray(res.conf_int()[1], dtype=float),
        "p-value": np.asarray(res.pvalues, dtype=float),
    })
    coef_df.to_csv(os.path.join(outdir, "gee_combined_coefficients.csv"), index=False)
    # Overview (sample size) like step 8
    overview = pd.DataFrame([
        {"metric": "total_people", "value": int(df["patient_id"].nunique()) if "patient_id" in df.columns else int(len(df))},
        {"metric": "total_observations", "value": int(len(df))},
    ])
    overview.to_csv(os.path.join(outdir, "gee_combined_overview.csv"), index=False)
    try:
        res.cov_params().to_csv(os.path.join(outdir, "gee_combined_covariance_matrix.csv"))
    except Exception as exc:
        logging.warning(
            "could not write gee_combined_covariance_matrix.csv (%s); the model summary and coefficients are unaffected",
            exc,
        )
    ...
    a1c_cats = list(df["baseline_a1c_category"].cat.categories) if "baseline_a1c_category" in df.columns and hasattr(df["baseline_a1c_category"], 'cat') else a1c_order
    ...
    for d in times:
        ...
        ax.set_title(
            f"Predicted A1C Change by Baseline A1C Category (Day {d})" f"{title_suffix}"
        )
    ...
    combined = preds.copy()
    # Ensure consistent order by the canonical A1C order
    combined["group_order"] = combined["a1c_group"].apply(lambda x: a1c_cats.index(x) if x in a1c_cats else 999)
    ...
    ax.axvline(0, color="black", linestyle="--", linewidth=1)
    ax.set_xlabel("Predicted Absolute A1C Change")
    ax.set_title("Predicted A1C Change by Baseline A1C Category and Time")
    ax.grid(True, axis="x", linestyle="--", alpha=0.5)
    ...
    # Uses the same four time points and inserts a blank spacer between clusters
    ...
    if not stacked_gbd.empty:
        ...
        ax.axvline(0, color="black", linestyle="--", linewidth=1)
        ax.set_xlabel("Predicted Absolute A1C Change")
        ax.set_title("Predicted A1C Change at 90 / 180 / 365 / 548 Days")
        ax.grid(True, axis="x", linestyle="--", alpha=0.5)
        ax.legend(title="Time from Baseline", bbox_to_anchor=(1.02, 1), loc="upper left")
        ax.invert_yaxis()
    ...
    for d in times:
        ...
        labels = list(sub["a1c_group"])
        texts = [
            f"{row['diff']:.2f} ({row['ci_low']:.2f}, {row['ci_high']:.2f})  n={int(preds[(preds['day'] == d) & (preds['a1c_group'] == row['a1c_group'])]['n_unique_window'].values[0]) if not preds[(preds['day'] == d) & (preds['a1c_group'] == row['a1c_group'])].empty else ''}"
            for _, row in sub.iterrows()
        ]
        ...
        n_all = counts_overall.get(d)
        title_suffix = f" (n={n_all})" if n_all is not None else ""
        ax.set_xlabel(f"Difference vs {ref_cat} (Absolute A1C Change)")
        ax.set_title(
