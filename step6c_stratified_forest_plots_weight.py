...
matplotlib.use("Agg")
"""
Step 6c: Stratified trajectories and forest plots (Weight outcome)

- For each covariate (sex, age group variants, race, metformin at baseline, BMI):
  ...
  - Generate Step 6-style trajectory plots (uncentered + centered) overlaying baseline A1C curves within subgroup.
  - Generate forest plots at requested time points.
  - Save predictions and contrasts CSVs per subgroup.
- Additionally, create combined plots.
"""
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Step 6c: Stratified trajectories and forest plots (Weight outcome)")
    p.add_argument(
        "--input-csv",
        default=os.path.join(
            "output",
            "step1_prepare_analysis_dataset",
            "analysis_ready_gap90.csv",
        ),
        help="Path to analysis-ready weight CSV from Step 1 (weight pipeline)",
    )
    p.add_argument(
        "--config-json",
        default=os.path.join("output", "step2_select_spline_df", "model_config.json"),
        help="Model config JSON from Step 2 (Weight, contains best df)",
    )
    p.add_argument(
        "--outdir",
        default=os.path.join("output", "step6c_stratified_by_covariates_weight"),
    ...
    p.add_argument("--xmax-cap", type=float, default=5.0, help="Optional max x-axis limit to truncate extreme CIs (default 5.0)")
    p.add_argument("--xmin-cap", type=float, default=-15.0, help="Optional min x-axis limit to truncate extreme CIs (default -15.0)")
    p.add_argument(
...
# Only produce trajectory plots for these stratification variables
TRAJECTORY_VARS = {"gender"}
...
def _fit_combined_stratum(df: pd.DataFrame, df_spline: int, exclude_var: str):
    ...
    # Interact spline with baseline A1C category to get category-specific trajectories (weight outcome)
    formula = f"pct_weight_change ~ {bs_term} * baseline_a1c_category"
    ...
    covariates = filter_estimable(
        ...
        context=f"step6c weight, exclude_var={exclude_var}",
    )
    if covariates:
...
def main(argv=None):
    ...
    df = enforce_a1c_order(df, order=A1C_ORDER, context="step6c_stratified_forest_plots_weight")
    ...
    # Cap time points at requested horizon (e.g., 548 days)
    ...
    passes = [
        (outdir, _fit_combined_stratum, "two-way"),
        (outdir_main, _fit_main_effects_stratum, "main-effects"),
    ]
    ...
    for curr_outdir, fit_fun, mode_label in passes:
        ...
        for var, var_label in STRAT_VARS:
            ...
            with open(os.path.join(var_base_outdir, "README.txt"), "w") as rf:
                rf.write(f"Stratified outputs for {var_label} ({mode_label}). Trajectories and forests generated by step6c (weight).\n")
        combined_records = []
        logging.info("Generating stratified outputs (%s) into %s (weight)", mode_label, curr_outdir)
        ...
        for var, var_label in STRAT_VARS:
            ...
            for subgroup in subgroups:
                ...
                # Early gender-only trajectories (two-way pass) before forests
                if mode_label == "two-way" and var == "gender":
                    try:
                        ...
                        for cat in a1c_cats:
                            ...
                            Xp_full = build_design_matrices([design_info], pred_df)[0]
                            Xp_full = np.asarray(Xp_full)
                            mean_u = np.asarray(res.predict(Xp_full), dtype=float)
                            anchor = mean_u[0]
                        ...
                        plt.title(f"{var_label}: {subgroup} — Weight Trajectories (Uncentered)\nstarting sample n = {int(n_people)}")
                        plt.axhline(0, color="black", linestyle="--", linewidth=1)
                        plt.xlim(0, int(getattr(args, "max_days", 548)))
                        plt.legend(); plt.grid(True, linestyle="--", alpha=0.7); plt.tight_layout()
                        out_unc = os.path.join(os.path.join(os.path.join(curr_outdir, f"by_{var}"), "trajectories"), "uncentered")
                        os.makedirs(out_unc, exist_ok=True)
                        plt.savefig(os.path.join(out_unc, f"trajectory_by_a1c_{var}_{str(subgroup).replace(' ', '_')}.png"), dpi=150)
                        plt.close()
                        # Save centered
                        plt.figure(figsize=(10, 6))
                        ...
                        plt.title(f"{var_label}: {subgroup} — Weight Trajectories\nstarting sample n = {int(n_people)}")
                        plt.axhline(0, color="black", linestyle="--", linewidth=1)
                        plt.xlim(0, int(getattr(args, "max_days", 548)))
                        plt.legend(); plt.grid(True, linestyle="--", alpha=0.7); plt.tight_layout()
                        out_ctr = os.path.join(os.path.join(os.path.join(curr_outdir, f"by_{var}"), "trajectories"), "centered")
                        os.makedirs(out_ctr, exist_ok=True)
                        plt.savefig(os.path.join(out_ctr, f"trajectory_by_a1c_{var}_{str(subgroup).replace(' ', '_')}.png"), dpi=150)
                        plt.close()
                    except Exception as e:
                        logging.warning("Early weight trajectories failed for %s=%s: %s", var_label, subgroup, e)
                        continue
                ...
                for d in times_in_support:
                    ...
                    for cat in a1c_cats:
                        if cat == ref_cat:
                            continue
                        df_ref = pd.DataFrame({"days_from_baseline": [d], "baseline_a1c_category": [ref_cat]})
                        df_cat = pd.DataFrame({"days_from_baseline": [d], "baseline_a1c_category": [cat]})
                        for c, v in mode_vals.items():
                        ...
                        contrast_rows.append({"day": d, "ref": ref_cat, "a1c_group": cat, "diff": diff, "ci_low": lo, "ci_high": hi, "se": se})
                ...
                # Per-day forest plots (predictions) — with right-side text column
                for d in times_in_support:
                    ...
                    ax.set_title(f"{var_label}: {subgroup} — Weight (Day {d}) [{mode_label}]")
                ...
                # Combined forest plot across all time points (predictions)
                combined = preds.copy()
                ...
                if not stacked.empty:
                    ...
                    ax.axvline(0, color="black", linestyle="-", linewidth=1)
                    ax.set_xlabel("Predicted Percent Weight Change")
                    ax.set_title(f"{var_label}: {subgroup} — Weight by Category and Time [{mode_label}]")
                    ax.grid(False)
                    ...
                    plt.tight_layout()
                    fig.savefig(
                        os.path.join(subgroup_dir, "forest_predictions_all_days.png"), dpi=150, bbox_inches='tight'
                    )
                    plt.close(fig)

                # Grouped-by-day forest plot for 12 and ~18 months — based on available days
                selected_12 = 365
                possible_18 = [548, 547, 540]
                day_values = sorted(combined["day"].unique().tolist())
                ...
                if selected_12 in day_values and selected_18 is not None:
                    ...
                    if not stacked_gbd.empty:
                        ...
                        for d in day_clusters:
                            subp = stacked_gbd[stacked_gbd["day"] == d].sort_values("y_index")
                            ax.errorbar(
                                subp["pred"],
                                subp["y_index"],
                                xerr=[subp["pred"] - subp["ci_low"], subp["ci_high"] - subp["pred"]],
                        ...
                        y_base = 0
                        for d in day_clusters:
                            for cat_idx, cat in enumerate(A1C_ORDER):
                        ...
                        ax.axvline(0, color="black", linestyle="-", linewidth=1)
                        ax.set_xlabel("Predicted Percent Weight Change")
                        ax.set_title(f"{var_label}: {subgroup} — Weight at 12 and 18 Months (Grouped) [{mode_label}]")
                        ax.grid(False)
                        ax.invert_yaxis()
        ...
        if combined_records:
            ...
            if combined_records:
                ...
                if not stacked_day.empty:
                    ...
                    ax = fig.add_subplot(gs[0, 0])
                    ax_txt = fig.add_subplot(gs[0, 1])
                    colors_map = {selected_days[0]: "tab:blue"}
                    if len(selected_days) > 1:
                        colors_map[selected_days[1]] = "tab:orange"
                    for d in selected_days:
                ...
                # Trajectory plots by baseline A1C within this subgroup (two-way only)
                if mode_label == "two-way" and var in TRAJECTORY_VARS:
                    try:
                        ...
                        for cat in a1c_cats:
                            ...
                            mean_c = mean_u - anchor
                            overlay.append((cat, days_grid, mean_u, mean_c))
                        logging.info("Weight trajectories: %s=%s overlay groups=%d", var_label, subgroup, len(overlay))
                        # Uncentered plot
                        plt.figure(figsize=(10, 6))
                        for cat, xg, mu, _ in overlay:
                            v = np.isfinite(mu)
                        ...
                        plt.tight_layout()
                        out_path_traj = os.path.join(traj_unc, f"trajectory_by_a1c_{var}_{str(subgroup).replace(' ', '_')}.png")
                        os.makedirs(os.path.dirname(out_path_traj), exist_ok=True)
                        plt.savefig(out_path_traj, dpi=150)
                        plt.close()
                        logging.info("Saved weight uncentered trajectory: %s", out_path_traj)
                        # Centered plot
                        plt.figure(figsize=(10, 6))
                        for cat, xg, _, mc in overlay:
                            v = np.isfinite(mc)
                        ...
                        plt.grid(True, linestyle="--", alpha=0.7)
                        plt.tight_layout()
                        out_path_traj = os.path.join(traj_ctr, f"trajectory_by_a1c_{var}_{str(subgroup).replace(' ', '_')}.png")
                        os.makedirs(os.path.dirname(out_path_traj), exist_ok=True)
                        plt.savefig(out_path_traj, dpi=150)
                        plt.close()
                        logging.info("Saved weight centered trajectory: %s", out_path_traj)
                        ...
                        plt.xlabel("Days from Baseline")
                        plt.ylabel("Percent Weight Change (%)")
                        plt.title(f"{var_label}: {subgroup} — Grouped Weight Trajectories (Uncentered)")
                        plt.axhline(0, color="black", linestyle="--", linewidth=1)
                        plt.legend()
                        plt.grid(True, linestyle="--", alpha=0.7)
                        plt.tight_layout()
                        out_path_traj = os.path.join(traj_grp, f"grouped_trajectory_by_a1c_{var}_{str(subgroup).replace(' ', '_')}.png")
                        os.makedirs(os.path.dirname(out_path_traj), exist_ok=True)
                        plt.savefig(out_path_traj, dpi=150)
                        plt.close()
                        logging.info("Saved weight grouped trajectory: %s", out_path_traj)
                    except Exception as e:
                        logging.warning("Weight trajectories failed for %s=%s: %s", var_label, subgroup, e)
