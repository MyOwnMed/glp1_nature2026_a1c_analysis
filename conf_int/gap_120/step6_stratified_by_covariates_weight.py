#!/usr/bin/env python3
"""
Step 6: Stratified GEE analyses by key covariates, controlling for baseline A1C (Weight)

Audience-friendly summary:
- We create separate analyses for subgroups (age group, sex, race, GLP-1+metformin at baseline, BMI).
- Within each subgroup, we estimate percent weight change over time.
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Step 6: Stratified GEE by covariates (Weight)")
    p.add_argument(
        "--input-csv",
    ...
    p.add_argument(
        "--outdir",
        default=os.path.join("output", "step6_stratified_by_covariates_weight"),
...
def main(argv=None):
    ...
    df = enforce_a1c_order(df, order=A1C_ORDER, context="step6_stratified_by_covariates_weight")
    ...
    for var, var_label in STRAT_VARS:
        ...
        for d in (var_outdir, os.path.join(var_outdir, "plots"), plots_unc, plots_ctr, plots_grp, models_dir):
        ...
        for subgroup in subgroups:
            ...
            try:
                ...
                covariates = filter_estimable(
                    ...
                    context=f"step6 stratified weight, var={var}",
                )
                bs_term = f"bs(days_from_baseline, df={df_spline})"
                formula = "pct_weight_change ~ " + bs_term
                if covariates:
            ...
            # Plot a single solid colored line through the entire time window
            if np.any(v):
                (line,) = plt.plot(
                    days_grid[v],
                    mean_u[v],
            ...
            plt.ylim(-10, 4)
            plt.title(
                f"{var_label}: {subgroup} - Percent Weight Change Over Time (Uncentered){gap_suffix}\nstarting sample n = {n_people}"
            )
            ...
            plt.ylim(-10, 4)
            plt.title(
                f"{var_label}: {subgroup} - Percent Weight Change Over Time{gap_suffix}\nstarting sample n = {n_people}"
            )
            plt.axhline(0, color="black", linestyle="--", linewidth=1)
            plt.xlim(0, args.max_days)
            plt.grid(True, linestyle="--", alpha=0.7)
        ...
        if overlay:
            ...
            ax.set_ylabel("Percent Weight Change (Centered to Day 0)")
            ax.set_ylim(-10, 4)
            if not args.suppress_grouped_titles:
                ax.set_title(
                    f"Percent Weight Change by {var_label}{gap_suffix}\n(n\u2009=\u2009{total_n})"
                )
