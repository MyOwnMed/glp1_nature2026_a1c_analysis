...
matplotlib.use("Agg")
"""
Step 6cc: 3-way stratification (Weight outcome)
Curves of percent weight change over time stratified by Gender × Age (<40 vs 40+).
- Input: Step 1 weight analysis-ready CSV (gap-specific)
- Output: Single trajectory per stratum (uncentered and centered) and combined overlays across strata
- Gap routing: by default routes outputs into output/gap_120/step6cc_3way_by_age_sex_weight
"""
import os
import argparse
import logging
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Step 6cc: 3-way stratified trajectories (Weight)")
    p.add_argument(
        "--input-csv",
        default=os.path.join(
            "output",
            "step1_prepare_analysis_dataset",
            "analysis_ready_gap120.csv",
        ),
        help="Path to analysis-ready weight CSV from Step 1 (gap=120 by default)",
    )
    p.add_argument(
        "--outdir",
        default=os.path.join("output", "step6cc_3way_by_age_sex_weight"),
...
def _fit_stratum(df: pd.DataFrame, df_spline: int, exclude_vars: list):
    ...
    covariates = filter_estimable(
        ...
        context=f"step6cc weight, exclude_vars={list(exclude_vars)}",
    )
    if covariates:
...
def main(argv=None):
    ...
    with open(os.path.join(outdir, "RUN_OK.txt"), "w") as f:
        f.write("step6cc_weight started\n")
    ...
    df = enforce_a1c_order(df, order=A1C_ORDER, context="step6cc_3waystrat_covariates_weight")
    ...
    with open(os.path.join(outdir, "README.txt"), "w") as rf:
        rf.write("3-way stratified Weight trajectories by baseline A1C, stratified by Gender x Age (<40 vs 40+).\n")
    ...
    combined_strata = []
    # Track quick per-stratum observed day-window means for later summary CSV
    summary_records = []
    # Key timepoints (filtered by grid range later)
    tpoints_core = [0, 180, 365, 548]
    ...
    for g in genders:
        for ab in age_bins:
            ...
            # Counts at timepoints
            grid_max_local = int(days_grid.max()) if hasattr(days_grid, 'max') else max(days_grid)
            tpoints = [t for t in tpoints_core if t <= grid_max_local]
            dd = pd.to_numeric(sub.get("days_from_baseline"), errors="coerce").to_numpy()
            ids = sub.get("patient_id").to_numpy() if "patient_id" in sub.columns else np.arange(len(sub))
            counts = {}
            for t in tpoints:
                sel = (dd >= t - args.window_days) & (dd <= t + args.window_days) & np.isfinite(dd)
                counts[str(t)] = int(np.unique(ids[sel]).size) if np.any(sel) else 0
            ...
            plt.xlabel("Days from Baseline"); plt.ylabel("Percent Weight Change (%)")
            plt.title(f"{title_core} — Weight Trajectory (Uncentered)\nstarting sample n = {int(n_people)}")
            plt.axhline(0, color="black", linestyle="--", linewidth=1)
            plt.xlim(0, int(getattr(args, "max_days", 548)))
            plt.legend(); plt.grid(True, linestyle="--", alpha=0.7); plt.tight_layout()
            out_path = os.path.join(traj_unc, f"weight_traj_gender_{_sanitize(str(g))}_age_{_sanitize(str(ab))}.png")
            ...
            plt.xlabel("Days from Baseline"); plt.ylabel("Percent Weight Change (Centered)")
            plt.title(f"{title_core} — Weight Trajectory")
            plt.axhline(0, color="black", linestyle="--", linewidth=1)
            plt.xlim(0, int(getattr(args, "max_days", 548)))
            plt.legend(); plt.grid(True, linestyle="--", alpha=0.7); plt.tight_layout()
            out_path = os.path.join(traj_ctr, f"weight_traj_gender_{_sanitize(str(g))}_age_{_sanitize(str(ab))}.png")
            os.makedirs(os.path.dirname(out_path), exist_ok=True)
            plt.savefig(out_path, dpi=150); plt.close()
            combined_strata.append({"label": f"{g} | {ab}", "gender": str(g), "age_bin": str(ab), "x": days_grid, "y_unc": mu, "y_ctr": mc,
                                     "ci_low_u": ci_low_u, "ci_high_u": ci_high_u,
                                     "ci_low_c": ci_low_c, "ci_high_c": ci_high_c})
            # Store gender and age_bin for targeted overlays
            combined_strata[-1]["gender"] = str(g)
            combined_strata[-1]["age_bin"] = str(ab)
            combined_strata[-1]["counts"] = counts

            # Summaries at key days (observed windows and predicted)
            for target in tpoints:
                # Observed window mean and N unique
                dd = pd.to_numeric(sub.get("days_from_baseline"), errors="coerce").to_numpy()
                ww = pd.to_numeric(sub.get("pct_weight_change"), errors="coerce").to_numpy()
                ids = sub.get("patient_id").to_numpy() if "patient_id" in sub.columns else np.arange(len(sub))
                sel = (dd >= target - 28) & (dd <= target + 28) & np.isfinite(dd) & np.isfinite(ww)
                obs_mean = float(np.nanmean(ww[sel])) if np.any(sel) else np.nan
                n_unique = int(np.unique(ids[sel]).size) if np.any(sel) else 0
                # Predicted at nearest grid day
                idx = int(np.argmin(np.abs(days_grid - target)))
                pred_uc = float(mu[idx]) if idx < len(mu) else np.nan
                pred_c = float(mc[idx]) if idx < len(mc) else np.nan
                summary_records.append({
                    "gender": str(g),
                    "age_bin": str(ab),
                    "target_day": int(target),
                    "observed_mean_pct_weight_change": obs_mean,
                    "observed_n_unique": n_unique,
                    "pred_uncentered": pred_uc,
                    "pred_centered": pred_c,
                    "n_obs": int(n_obs),
                    "n_people": int(n_people),
                })

    if combined_strata:
        # Compute common y-limits and plotting helpers
        ...
        plt.xlabel("Days from Baseline"); plt.ylabel("Percent Weight Change (%)")
        plt.title("Weight — Overlay by Sex × Age (<40 vs 40+)")
        plt.axhline(0, color="black", linestyle="--", linewidth=1)
        plt.legend(); plt.grid(True, linestyle="--", alpha=0.7); plt.tight_layout()
        ...
        if male_series:
            ...
            for s in male_series:
                ...
                if np.any(v):
                    ...
                    _scatter_markers(xg, mu, line.get_color())
            plt.xlabel("Days from Baseline"); plt.ylabel("Percent Weight Change (%)")
            plt.title("Weight — Male Overlay by Age (<40 vs 40+)")
            plt.axhline(0, color="black", linestyle="--", linewidth=1)
        ...
        if female_series:
            ...
            for s in female_series:
                ...
                if np.any(v):
                    ...
                    _scatter_markers(xg, mu, line.get_color())
            plt.xlabel("Days from Baseline"); plt.ylabel("Percent Weight Change (%)")
            plt.title("Weight — Female Overlay by Age (<40 vs 40+)")
            plt.axhline(0, color="black", linestyle="--", linewidth=1)
            plt.legend(title="Age Bin"); plt.grid(True, linestyle="--", alpha=0.7); plt.tight_layout()
        ...
        try:
            ...
            out_csv = os.path.join(traj_combined, "overlay_strata_summary.csv")
            summ_df.to_csv(out_csv, index=False)
            logging.info("Wrote summary CSV: %s", out_csv)
        except Exception as e:
            logging.warning("Could not write overlay_strata_summary.csv: %s", e)
