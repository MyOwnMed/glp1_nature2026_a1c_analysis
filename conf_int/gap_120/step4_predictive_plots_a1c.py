#!/usr/bin/env python3
"""
Step 4 (A1C outcome): Predictive plots of absolute A1C change over time by baseline A1C category.

This mirrors the reference implementation you provided and reads from
the A1C-specific Step 1 dataset and Step 2 config.
"""
...
CI_MAX_ABS_WIDTH = float(os.getenv("PLOT_CI_MAX_ABS_WIDTH", "1.0"))
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Step 4 (A1C): Predictive plots by baseline A1C category")
    p.add_argument("--input-csv", default=os.path.join("output", "step1_prepare_analysis_dataset_a1c", "analysis_ready_a1c_gap90.csv"), help="Path to A1C analysis-ready CSV from Step 1 (A1C)")
    p.add_argument("--config-json", default=os.path.join("output", "step2_select_spline_df_a1c", "model_config_a1c.json"), help="Model config JSON from Step 2 (A1C)")
    p.add_argument("--outdir", default=os.path.join("output", "step4_predictive_plots_a1c"), help="Directory to write plots")
...
def _fit_gee(sub_df: pd.DataFrame, df_spline: int):
    covariates = ["age_group", "gender", "baseline_bmi_final_category", "race"]
    if "weight_change_med" in sub_df.columns:
        covariates.append("weight_change_med")
    covariates = filter_estimable(covariates, sub_df, context="step4 predictive a1c")
    bs_term = f"bs(days_from_baseline, df={df_spline})"
    formula = f"abs_a1c_change ~ {bs_term}" + (" + " + " + ".join(covariates) if covariates else "")
...
def main(argv=None):
    ...
    for subgroup in strata:
        ...
        summ_path = os.path.join(summaries_out, f"gee_summary_a1c_outcome_a1c_{str(subgroup).replace(' ', '_')}.txt")
        with open(summ_path, 'w') as f:
            f.write("Outcome: abs_a1c_change (percentage-points)\n")
        ...
        plt.plot(x_vals, pred_mean_u, label=_legend_label_for_a1c_category(subgroup), color=color)
        plt.xlabel('Days from Baseline')
        plt.ylabel('Absolute A1C Change (percentage-points)')
        plt.ylim(-3, 1)
        plt.title(f'A1C: {subgroup} - Absolute A1C Change Over Time (Uncentered){gap_suffix}\nstarting sample n = {n_people}')
        plt.axhline(0, color='black', linestyle='--', linewidth=1)
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.tight_layout()
        out_path = os.path.join(uncentered_out, f"{a1c_rank.get(subgroup,99)+1:02d}_trajectory_a1c_outcome_a1c_{str(subgroup).replace(' ', '_')}.png")
        ...
        plt.ylim(-3, 1)
        plt.title(f'A1C: {subgroup} - Absolute A1C Change Over Time{gap_suffix}\nstarting sample n = {n_people}')
        plt.axhline(0, color='black', linestyle='--', linewidth=1)
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.tight_layout()
        out_path = os.path.join(centered_out, f"{a1c_rank.get(subgroup,99)+1:02d}_trajectory_a1c_outcome_a1c_{str(subgroup).replace(' ', '_')}.png")
        plt.savefig(out_path, dpi=300)
        plt.close()
    ...
    if not args.suppress_grouped_titles:
        plt.title(f'Baseline A1C Category - Grouped Trajectories (Uncentered, A1C outcome){gap_suffix}\nstarting sample n = {total_n}')
    plt.axhline(0, color='black', linestyle='--', linewidth=1)
    if not args.suppress_grouped_legend:
        plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig(os.path.join(grouped_out, "grouped_trajectory_uncentered_outcome_a1c.png"), dpi=300)
    plt.close()
    ...
    for subgroup in legend_order:
        ...
        if zero_idx.size > 0:
            ...
            mg[zero_idx[0]] = 0.0
        plt.plot(xg, mg, label=res['label'], color=color)
        # 95% CI bands on grouped centered overlay
        if SHOW_CI:
    ...
    plt.ylim(-3, 1)
    if not args.suppress_grouped_titles:
        plt.title(f'Baseline A1C Category - Grouped Trajectories (A1C outcome){gap_suffix}\nstarting sample n = {total_n}')
    plt.axhline(0, color='black', linestyle='--', linewidth=1)
    if not args.suppress_grouped_legend:
        plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    centered_path = os.path.join(grouped_out, "grouped_trajectory_centered_outcome_a1c.png")
    ...
    if args.write_grouped_legend_only:
        ...
        if handles:
            ...
            legend_path = os.path.join(grouped_out, "grouped_trajectory_legend_a1c.png")
    ...
    if tables:
        pd.concat(tables, ignore_index=True).to_csv(os.path.join(tables_out, "predicted_means_by_day_a1c_outcome_a1c.csv"), index=False)
