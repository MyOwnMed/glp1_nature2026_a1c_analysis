...
def fit_gee_baseline(
    input_csv: Path,
    config_json: Path,
    outdir: Path,
    drop_covariates: Optional[Sequence[str]] = None,
) -> None:
    logging.info("Reading weight analysis CSV %s", input_csv)
    ...
    df_model = df.dropna(subset=["pct_weight_change", "days_from_baseline"]).copy()
    if df_model.empty:
        raise ValueError("No rows with non-missing pct_weight_change and days_from_baseline")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Step 3 (weight): Fit baseline GEE model for pct_weight_change",
    )
    parser.add_argument(
        "--input-csv",
        required=True,
        help="Weight analysis CSV (e.g., output/step1_prepare_analysis_dataset/analysis_ready_gap90.csv)",
    )
    parser.add_argument(
        "--config-json",
        required=True,
        help="Model config JSON from Step 2 (e.g., output/step2_select_spline_df/model_config.json)",
    )
    parser.add_argument(
        "--outdir",
        required=False,
        default="output/step3_fit_gee_baseline",
        help="Output directory for fitted GEE model results",
    )
    parser.add_argument(
        "--adherence-gap-days",
        type=int,
