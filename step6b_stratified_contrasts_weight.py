#!/usr/bin/env python3
"""
Step 6b: Stratified contrasts at specified days (Weight outcome)
...
- For each stratification variable (age, sex, race, baseline BMI, baseline metformin),
  ...
  and compute contrasts (difference in predicted percent weight change) at target days.
- Contrasts are absolute differences in percentage points vs a chosen reference subgroup.
- 95% CIs use se_diff = sqrt(se_ref^2 + se_sub^2) assuming independent subgroup fits.

Inputs:
- Step 1 weight analysis-ready CSV
- Step 2 weight spline df config JSON

Outputs:
- CSV per strat_var listing contrasts vs reference at each target day:
  output/step6b_stratified_contrasts_weight/contrasts_<strat_var>.csv
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Step 6b: Stratified contrasts at target days (Weight)")
    p.add_argument(
        "--input-csv",
    ...
    p.add_argument(
        "--outdir",
        default=os.path.join("output", "step6b_stratified_contrasts_weight"),
...
def _fit_subgroup_model(sub_df: pd.DataFrame, df_spline: int, strat_var: str):
    ...
    covariates = filter_estimable(
        ...
        context=f"step6b weight, strat_var={strat_var}",
    )
...
def main(argv=None):
    ...
    df = enforce_a1c_order(df, context="step6b_stratified_contrasts_weight")
